...
| Sources | `FilesystemSource`, `ObsidianSource`, `NotionSource`, `FetchedSource`. Two stages: `discover()` opens nothing, `read()` opens one thing — except `FetchedSource`, which opens the `.fetch.json` records at discovery because deduplication is discovery's answer to give, and never a page ([`sources.md`](sources.md)). Notion keys by the **page id** and skips anything without one, so `key_derivation` is true of every unit in a run ([`sources.md`](sources.md)) |
| Converter floors | Per converter and per measure, set **below** what each was measured at and printed either way (`tools/floors.py`, [`measurements.md`](measurements.md)). Floors, not targets: a gate at today's number makes an honest experiment a build failure |
...
| Converter conformance | **One suite every registered converter must pass**, whoever wrote it — the tiling, the verbatim rule, no offset outside its source, determinism, refusal rather than a raise. Parametrised over the registry, so a converter added by a third party is held to it without anybody adding a test. Verified potent: a deliberately broken converter fails 9 of 9 |
...
| `musubi errors` | **The closed set of names musubi prints before the first colon on standard error**, with the exit code, whether asking again could help, and one line each in English and Japanese ([ADR-0046](adr/0046-a-failure-that-a-program-can-name.md)). For a program that folds failures by name: every failure used to print `musubi: ...`, so all of them folded into one bucket called `musubi`. The catalogue is checked against the code — a kind that can reach standard error and is not in it fails the build |
